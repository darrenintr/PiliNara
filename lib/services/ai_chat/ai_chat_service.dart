...
class AiChatService {
  ...
  static AiApiException _logged(AiApiException ex, [StackTrace? stackTrace]) {
    logger.e('AI 请求失败'.tr, error: ex, stackTrace: stackTrace);
    return ex;
  }
  ...
  static Future<AiApiException> _requestError(String url, DioException e) async {
    ...
    if (e.type == DioExceptionType.badResponse) {
      ...
      if (detail.isEmpty) detail = '服务器返回错误'.tr;
    } else {
      detail = switch (e.type) {
        ...
        DioExceptionType.receiveTimeout =>
          '连接超时'.tr,
        DioExceptionType.badCertificate => '证书校验失败'.tr,
        DioExceptionType.cancel => '请求已取消'.tr,
  ...
  static Future<List<String>> fetchModels() async {
    ...
    if (baseUrl.isEmpty) throw Exception('请先配置 API 地址'.tr);
    ...
    throw _logged(AiApiException(
      ...
      detail: '响应不是模型列表，请检查接口地址与版本路径'.tr +
  ...
  static Stream<String> streamChat({
    ...
    if (useModel.isEmpty) throw Exception('请先选择模型'.tr);
    ...
    if (!sawData) {
      ...
      throw _logged(AiApiException(
        ...
        detail: '未返回流式响应，请检查接口地址与版本路径'.tr +
  ...
  static final List<AiPromptTemplate> defaultTemplates = [
    AiPromptTemplate(
      ...
      prompt: '请对这个视频内容进行概貌总结。考虑到视频可能较长，请避免过度省略。\n'.tr +
          ...
          '4. 结构必须极其清晰，便于用户在 10 秒内判断是否值得观看全片。'.tr,
    ),
    AiPromptTemplate(
      name: '详细分析'.tr,
      prompt: '请对这个视频进行极具深度的拆解分析。请克服长文本的省略倾向，尽可能保留具体细节、案例和逻辑推演。\n'.tr +
          ...
          '注意：全程必须高频使用 `[mm:ss]` 时间戳进行锚定，以便我随时点击溯源。'.tr,
    ),
  ];
  ...
  static List<AiPromptTemplate> getTemplates() {
    ...
    try {
      ...
      templates.removeWhere((t) => t.name == '准备问答'.tr);
