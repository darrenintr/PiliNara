...
class WhisperLinkSettingController extends GetxController {
  ...
  void setPush(bool isPush) {
    if (isPush) {
      showConfirmDialog(
        context: Get.context!,
        title: Text('确认关闭内容推送吗？'.tr),
        content: Text('若关闭此开关，你将不再收到该账号的图文消息与稿件推送，但通知类消息不受影响'.tr),
  ...
  Future<void> setBlock(bool isBlocked) async {
    if (isBlocked) {
      ...
      showConfirmDialog(
        context: Get.context!,
        title: Text('确认拉黑该用户'.tr),
        content: Text('加入黑名单后，将自动解除关注关系和对该用户的合集订阅关系，禁止该用户与我互动或查看我的空间'.tr),
        onConfirm: () async {
          final res = await VideoHttp.relationMod(
            mid: talkerUid,
            act: 5,
