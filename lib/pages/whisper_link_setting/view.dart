...
class _WhisperLinkSettingPageState extends State<WhisperLinkSettingPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return SimpleScaffold(
      appBar: AppBar(title: Text('聊天设置'.tr)),
      body: ListView(
  ...
  Widget _buildBlockItem(bool isBlocked) {
    return ListTile(
      ...
      title: Text('加入黑名单'.tr, style: TextStyle(fontSize: 14)),
            ...
            children: [
              if (response.showPushSetting == 1)
                ListTile(
                  ...
                  title: Text('接收消息推送'.tr, style: TextStyle(fontSize: 14)),
                  subtitle: Text(
                    '若关闭此开关，你将不再收到该账号的图文消息与稿件推送，但通知类消息不受影响'.tr,
                    style: subTitleS,
                  ),
                  trailing: Transform.scale(
                    alignment: Alignment.centerRight,
              ...
              Obx(
                () => ListTile(
                  ...
                  title: Text('置顶聊天'.tr, style: TextStyle(fontSize: 14)),
                  trailing: Transform.scale(
                    alignment: Alignment.centerRight,
                    scale: 0.8,
                    child: Switch(
      ...
      Success(:final response) =>
        response != null && response.isNotEmpty
            ? ListTile(
                ...
                title: Text('消息免打扰'.tr, style: TextStyle(fontSize: 14)),
                trailing: Transform.scale(
                  alignment: Alignment.centerRight,
                  scale: 0.8,
                  child: Switch(
